...
def check_binaries():
    ...
    if not os.path.exists(C_DAEMON) or not os.path.exists(C_CTL):
        ...
        # Compile softflowd
        subprocess.run([
            "gcc", "-o", "softflowd", "freelist.c", "softflowd.c", "log.c", "netflow5.c", 
            "ipfix.c", "psamp.c", "netflow9.c", "netflow1.c", "convtime.c", "strlcpy.c", 
            "strlcat.c", "closefrom.c", "daemon.c", "-lpcap"
        ], cwd=C_DIR, check=True)
        # Compile softflowctl
        subprocess.run([
            "gcc", "-o", "softflowctl", "softflowctl.c", "convtime.c", "strlcpy.c", 
            "strlcat.c", "closefrom.c", "daemon.c"
        ], cwd=C_DIR, check=True)

    # Verify paths exist
    for path, name in [(C_DAEMON, "C daemon"), (C_CTL, "C control"), (RUST_DAEMON, "Rust daemon"), (RUST_CTL, "Rust control")]:
...
def create_dummy_pcap(filepath: str):
    ...
    eth_hdr = struct.pack("!6s6sH", b"\x02\x02\x02\x02\x02\x02", b"\x01\x01\x01\x01\x01\x01", 0x0800)
    ip_hdr = struct.pack("!BBHHHBBH4s4s", 0x45, 0, 40, 0x1234, 0, 64, 6, 0, socket.inet_aton("192.168.1.100"), socket.inet_aton("10.0.0.1"))
    tcp_hdr = struct.pack("!HHIIBBHHH", 12345, 80, 1000, 0, 0x50, 0x02, 65535, 0, 0) # SYN
    packets.append(eth_hdr + ip_hdr + tcp_hdr)
    
    # 2. TCP ACK from 10.0.0.1:80 -> 192.168.1.100:12345
    ip_hdr_rev = struct.pack("!BBHHHBBH4s4s", 0x45, 0, 40, 0x1235, 0, 64, 6, 0, socket.inet_aton("10.0.0.1"), socket.inet_aton("192.168.1.100"))
    tcp_hdr_rev = struct.pack("!HHIIBBHHH", 80, 12345, 1, 1001, 0x50, 0x10, 65535, 0, 0) # ACK
    packets.append(eth_hdr + ip_hdr_rev + tcp_hdr_rev)

    # 3. UDP from 192.168.1.100:5555 -> 8.8.8.8:53 (length 30)
    ip_hdr_udp = struct.pack("!BBHHHBBH4s4s", 0x45, 0, 38, 0x1236, 0, 64, 17, 0, socket.inet_aton("192.168.1.100"), socket.inet_aton("8.8.8.8"))
    ...
    ip_hdr_icmp = struct.pack("!BBHHHBBH4s4s", 0x45, 0, 28, 0x1237, 0, 64, 1, 0, socket.inet_aton("192.168.1.100"), socket.inet_aton("192.168.1.1"))
...
def test_cli_compatibility():
    ...
    if missing_c or missing_rust:
        print_red(f"FAILED: Help strings missing key options. C missing: {missing_c}, Rust missing: {missing_rust}")
        sys.exit(1)
    
    print_green("CLI compatibility tests PASSED.")

def start_blocked_daemon(daemon_bin: str, sock_path: str, extra_args: list = None) -> Tuple[subprocess.Popen, int, str, str]:
...
def run_cross_control_tests():
    ...
    try:
        ...
        out_c_c = subprocess.run([C_CTL, "-c", sock_c, "statistics"], capture_output=True, text=True).stdout
    ...
    try:
        ...
        out_rust_c = subprocess.run([RUST_CTL, "-c", sock_c, "statistics"], capture_output=True, text=True).stdout
        ...
        shutdown_res = subprocess.run([RUST_CTL, "-c", sock_c, "shutdown"], capture_output=True, text=True).stdout
    ...
    proc_rust, r_fifo_fd, r_fifo_path, r_fifo_dir = start_blocked_daemon(RUST_DAEMON, sock_rust)
    
    try:
        # Query Rust daemon statistics using C ctl
        out_c_rust = subprocess.run([C_CTL, "-c", sock_rust, "statistics"], capture_output=True, text=True).stdout
        print_yellow(f"C CTL -> Rust Daemon Statistics:\n{out_c_rust}")
        
        # Query Rust daemon statistics using Rust ctl
        out_rust_rust = subprocess.run([RUST_CTL, "-c", sock_rust, "statistics"], capture_output=True, text=True).stdout
        ...
        shutdown_res = subprocess.run([C_CTL, "-c", sock_rust, "shutdown"], capture_output=True, text=True).stdout
        print_yellow(f"C CTL -> Rust Daemon Shutdown: {shutdown_res.strip()}")
        
        proc_rust.wait(timeout=2)
...
def parse_netflow_v5(data: bytes) -> List[dict]:
    ...
    for _ in range(count):
        ...
        src, dst, nexthop, input_if, output_if, pkts, octets, first, last, src_port, dst_port, pad1, tcp_flags, prot, tos = struct.unpack(
            "!4s4s4sHHIIIHBsBB", rec[0:39]
        )
        flows.append({
...
def test_differential_packets():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for version in [5, 9, 10]:
            ...
            c_proc = subprocess.Popen([C_DAEMON, "-d", "-r", pcap_path, "-n", f"127.0.0.1:{c_port}", "-v", str(version), "-a"])
            c_proc.wait()
            ...
            rust_proc = subprocess.Popen([RUST_DAEMON, "-d", "-r", pcap_path, "-n", f"127.0.0.1:{rust_port}", "-v", str(version)])
            rust_proc.wait()
            ...
            print(f"  Captured {len(c_packets)} packets from C, {len(rust_packets)} packets from Rust")
            if not c_packets or not rust_packets:
                print_red(f"FAILED: No packets captured for version {version}. C: {len(c_packets)}, Rust: {len(rust_packets)}")
                sys.exit(1)
            ...
            if version == 5:
                ...
                def key_func(f):
                    return (f["src"], f["dst"], f["src_port"], f["dst_port"], f["protocol"])
                
                c_flows.sort(key=key_func)
                rust_flows.sort(key=key_func)
                
                if len(c_flows) != len(rust_flows):
                    print_red(f"FAILED: Number of parsed flows differs. C: {len(c_flows)}, Rust: {len(rust_flows)}")
                ...
                for i, (cf, rf) in enumerate(zip(c_flows, rust_flows)):
                    ...
                    for k in ["src", "dst", "src_port", "dst_port", "protocol", "tcp_flags"]:
                        if cf[k] != rf[k]:
                            print_red(f"FAILED: Flow record mismatch at index {i}, field '{k}': C={cf[k]}, Rust={rf[k]}")
                            print(f"C flow: {cf}")
                ...
                if v_c != version or v_r != version:
                    print_red(f"FAILED: Packet version header mismatch. Expected {version}, C got {v_c}, Rust got {v_r}")
                    sys.exit(1)
